   #include <stdio.h> 
   #include <stdlib.h>
   #include <string.h>
   #include <time.h>
   static const unsigned int Inverse_sbox[256] = {
      
   };
      static const unsigned char sbox[256] = {
      
   };

   void print_hex(unsigned char *t)
   {
   unsigned char i=0;

   for(i = 0; i<4; i++){
      printf("%02x ", t[i*4+0]);
      printf("%02x ", t[i*4+1]);
      printf("%02x ", t[i*4+2]);
      printf("%02x ", t[i*4+3]);
      printf("\n"); 
   }
   printf("\n");
   }

   void print_key(unsigned char *t)
   {
   unsigned char i,j=0;

   for (j=0; j<4; j++){
      for(i = 0; i<44; i++){
         printf("%02x ", t[j*44+i]);
         }
      printf("\n");
      }
      printf("\n");
   }


   
unsigned char MC(unsigned char src, unsigned int p){
   int i;
   unsigned char tmp[4];
   tmp [0] = src;
   
   for (i=1; i<4; i++){
      src = (src*2)^(src& 0x80 ? 0x1b : 0);
      tmp [i] = src;
   }
   
   switch (p) {
      case 2 : src = tmp[1]; break; //2
      case 3 : src = tmp[1]^tmp[0]; break; //2+1
      case 9 : src = tmp[3]^tmp[0]; break; //8+1
      case 0x0b : src = tmp[3] ^ tmp[1] ^ tmp[0] ;break; // 8+2+1
      case 0x0d : src = tmp[3]^tmp[2]^tmp[0]; break; //8+4+1
      case 0x0e : src = tmp[3]^tmp[2]^tmp[1]; break; // 8+4+2
   }
   
   return src;
}

   unsigned char W[176];
   void Key_Schedule(unsigned char* source){
   unsigned char Rcon[] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                     };

   char j;
   for(j=0; j<=3; j++){
      W[0*11*4+j] = source[0*4+j];
      W[1*11*4+j] = source[1*4+j];    
      W[2*11*4+j] = source[2*4+j];
      W[3*11*4+j] = source[3*4+j];
   }//4개를 집어 넣고 

   for (j=4; j<44; j++){
      if(j%4 ==0){
         W[0*11*4+j] = W[0*11*4+j-4]^sbox[W[1*11*4+j-1]]^Rcon[0*10+j/4-1];
         W[1*11*4+j] = W[1*11*4+j-4]^sbox[W[2*11*4+j-1]]^Rcon[1*10+j/4-1];    
         W[2*11*4+j] = W[2*11*4+j-4]^sbox[W[3*11*4+j-1]]^Rcon[2*10+j/4-1];
         W[3*11*4+j] = W[3*11*4+j-4]^sbox[W[0*11*4+j-1]]^Rcon[3*10+j/4-1];
      }
      else {
         W[0*11*4+j] = W[0*11*4+j-4]^ W[0*11*4+j-1];
         W[1*11*4+j] = W[1*11*4+j-4]^ W[1*11*4+j-1];    
         W[2*11*4+j] = W[2*11*4+j-4]^ W[2*11*4+j-1];
         W[3*11*4+j] = W[3*11*4+j-4]^ W[3*11*4+j-1];
      }
      }
      
      printf("key \n");
      print_key(W);
   }

   /*암호화 과정*/
   void AES_Encrypt(unsigned char * plainText, unsigned char * cipher_key, unsigned char*Encrypt_text){
   unsigned int i,j,k=0;

      Key_Schedule(cipher_key);
      i=0;
      for (j=0; j<4; j++){
         plainText[j*4+0] = plainText[j*4+0]^W[j*4*11+i*4+0];
         plainText[j*4+1] = plainText[j*4+1]^W[j*4*11+i*4+1];   
         plainText[j*4+2] = plainText[j*4+2]^W[j*4*11+i*4+2];
         plainText[j*4+3] = plainText[j*4+3]^W[j*4*11+i*4+3];
      }
      
   for (i=1; i<10; i++){
      for (j=0; j<4; j++){
         /*add round key 0k*/
         Encrypt_text[0*4+j] =  MC(sbox[(plainText[0*4+j])],2) ^ MC(sbox[(plainText[1*4+(j+1)%4])],3) ^   (sbox[(plainText[2*4+(j+2)%4])])   ^   (sbox[(plainText[3*4+(j+3)%4])])^W[0*4*11+i*4+j];
         Encrypt_text[1*4+j] =    (sbox[(plainText[0*4+j])])   ^ MC(sbox[(plainText[1*4+(j+1)%4])],2) ^ MC(sbox[(plainText[2*4+(j+2)%4])],3) ^   (sbox[(plainText[3*4+(j+3)%4])])^W[1*4*11+i*4+j];
         Encrypt_text[2*4+j] =    (sbox[(plainText[0*4+j])])   ^   (sbox[(plainText[1*4+(j+1)%4])])   ^ MC(sbox[(plainText[2*4+(j+2)%4])],2) ^ MC(sbox[(plainText[3*4+(j+3)%4])],3)^W[2*4*11+i*4+j];
         Encrypt_text[3*4+j] =  MC(sbox[(plainText[0*4+j])],3) ^   (sbox[(plainText[1*4+(j+1)%4])])   ^   (sbox[(plainText[2*4+(j+2)%4])])   ^ MC(sbox[(plainText[3*4+(j+3)%4])],2)^W[3*4*11+i*4+j];
      }
      for(k=0; k<4; k++){
         plainText[0*4+k] = Encrypt_text[0*4+k];
         plainText[1*4+k] = Encrypt_text[1*4+k];
         plainText[2*4+k] = Encrypt_text[2*4+k];
         plainText[3*4+k] = Encrypt_text[3*4+k];
         }
      // printf("%d Add Round Key\n",i);
      // print_hex(plainText);
   }
   /*9번째 end*/

      //printf ("10th \n");
      for (j=0; j<4; j++){
         Encrypt_text[0*4+j] = sbox[(plainText[0*4+j])]      ^W[0*4*11+i*4+j];
         Encrypt_text[1*4+j] = sbox[(plainText[1*4+(j+1)%4])]^W[1*4*11+i*4+j];
         Encrypt_text[2*4+j] = sbox[(plainText[2*4+(j+2)%4])]^W[2*4*11+i*4+j];
         Encrypt_text[3*4+j] = sbox[(plainText[3*4+(j+3)%4])]^W[3*4*11+i*4+j];
      }
      // print_hex(plainText);
   }

   /*복호화  과정*/
   void AES_Decrypt(unsigned char * Encrypt_text, unsigned char * cipher_key,unsigned char*Decrypt_text){
   unsigned int i,j,k=0;

      i=10;
       for (j=0; j<4; j++){
         /*round key 0k*/
         // Encrypt_text[0*4+j] = (Encrypt_text[0*4+j])^W[0*4*11+i*4+j];
         // Encrypt_text[1*4+j] = (Encrypt_text[1*4+j])^W[1*4*11+i*4+j];
         // Encrypt_text[2*4+j] = (Encrypt_text[2*4+j])^W[2*4*11+i*4+j];
         // Encrypt_text[3*4+j] = (Encrypt_text[3*4+j])^W[3*4*11+i*4+j];

         /*shift row 0k*/
         // Decrypt_text[0*4+j] = (Encrypt_text[0*4+j])      ^W[0*4*11+i*4+j];
         // Decrypt_text[1*4+j] = (Encrypt_text[1*4+(j-1)%4])^W[1*4*11+i*4+(j-1)%4];
         // Decrypt_text[2*4+j] = (Encrypt_text[2*4+(j-2)%4])^W[2*4*11+i*4+(j-2)%4];
         // Decrypt_text[3*4+j] = (Encrypt_text[3*4+(j-3)%4])^W[3*4*11+i*4+(j-3)%4];

         /*Inverse SubByte 0k*/
         Decrypt_text[0*4+j] = Inverse_sbox[(Encrypt_text[0*4+j])      ^W[0*4*11+i*4+j]];
         Decrypt_text[1*4+j] = Inverse_sbox[(Encrypt_text[1*4+(j-1)%4])^W[1*4*11+i*4+(j-1)%4]];
         Decrypt_text[2*4+j] = Inverse_sbox[(Encrypt_text[2*4+(j-2)%4])^W[2*4*11+i*4+(j-2)%4]];
         Decrypt_text[3*4+j] = Inverse_sbox[(Encrypt_text[3*4+(j-3)%4])^W[3*4*11+i*4+(j-3)%4]];

      }

      printf("Decrypt_Text add round key 10\n");
      print_hex(Decrypt_text);

   //  printf("key : %02x %02x %02x %02x \n",W[0*4*11+9*4+0],W[1*4*11+9*4+0],W[2*4*11+9*4+0],W[3*4*11+9*4+0]);  
      for (i=9; i>0; i--) {
         for(j=0; j<4; j++){
         
         /*add round key*/
         // Encrypt_text[0*4+j] = Decrypt_text[0*4+j]^W[0*4*11+i*4+j];
         // Encrypt_text[1*4+j] = Decrypt_text[1*4+j]^W[1*4*11+i*4+j];
         // Encrypt_text[2*4+j] = Decrypt_text[2*4+j]^W[2*4*11+i*4+j];
         // Encrypt_text[3*4+j] = Decrypt_text[3*4+j]^W[3*4*11+i*4+j];

        /*Inverse_Mix_Column*/
           // printf("%02x %02x %02x %02x ,%02x %02x %02x %02x ,%02x \n",Decrypt_text[0*4+j]^W[0*4*11+i*4+j], Decrypt_text[1*4+j]^W[1*4*11+i*4+j], Decrypt_text[2*4+j]^W[2*4*11+i*4+j], Decrypt_text[3*4+j]^W[3*4*11+i*4+j],  MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0E), MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0B),MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0D) , MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x09), MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0E) ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0B)^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0D) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x09));

         // Encrypt_text[0*4+j] = MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0E) ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0B)^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0D) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x09);
         // Encrypt_text[1*4+j] = MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x09) ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0E)^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0B) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x0D);
         // Encrypt_text[2*4+j] = MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0D) ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x09)^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0E) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x0B);
         // Encrypt_text[3*4+j] = MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0B) ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0D)^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x09) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x0E);

        /*Inverse_Shift_row*/
         //  printf("%02x %02x %02x %02x ,%02x %02x %02x %02x ,%02x \n",MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0E),MC(Encrypt_text[1*4+(j-1)%4]^W[1*4*11+i*4+(j-1)%4],0x0B),MC(Encrypt_text[2*4+(j-2)%4]^W[2*4*11+i*4+(j-2)%4],0x0D), MC(Encrypt_text[3*4+(j-3)%4]^W[3*4*11+i*4+(j-3)%4],0x09));
         // Encrypt_text[0*4+j] = MC(Decrypt_text[0*4+j]^W[0*4*11+i*4+j],0x0E) ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0B)^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0D) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x09);
         // Encrypt_text[1*4+j] = MC(Decrypt_text[0*4+(j+3)%4]^W[0*4*11+i*4+(j+3)%4],0x09) ^ MC(Decrypt_text[1*4+(j+3)%4]^W[1*4*11+i*4+(j+3)%4],0x0E)^MC(Decrypt_text[2*4+(j+3)%4]^W[2*4*11+i*4+(j+3)%4],0x0B) ^ MC(Decrypt_text[3*4+(j+3)%4]^W[3*4*11+i*4+(j+3)%4],0x0D);
         // Encrypt_text[2*4+j] = MC(Decrypt_text[0*4+(j+2)%4]^W[0*4*11+i*4+(j+2)%4],0x0D) ^ MC(Decrypt_text[1*4+(j+2)%4]^W[1*4*11+i*4+(j+2)%4],0x09)^MC(Decrypt_text[2*4+(j+2)%4]^W[2*4*11+i*4+(j+2)%4],0x0E) ^ MC(Decrypt_text[3*4+(j+2)%4]^W[3*4*11+i*4+(j+2)%4],0x0B);
         // Encrypt_text[3*4+j] = MC(Decrypt_text[0*4+(j+1)%4]^W[0*4*11+i*4+(j+1)%4],0x0B) ^ MC(Decrypt_text[1*4+(j+1)%4]^W[1*4*11+i*4+(j+1)%4],0x0D)^MC(Decrypt_text[2*4+(j+1)%4]^W[2*4*11+i*4+(j+1)%4],0x09) ^ MC(Decrypt_text[3*4+(j+1)%4]^W[3*4*11+i*4+(j+1)%4],0x0E);

        /*Inverse_sbox*/
         Encrypt_text[0*4+j] = Inverse_sbox[MC(Decrypt_text[0*4+j      ]^W[0*4*11+i*4+j],0x0E)       ^ MC(Decrypt_text[1*4+j]^W[1*4*11+i*4+j],0x0B)            ^MC(Decrypt_text[2*4+j]^W[2*4*11+i*4+j],0x0D) ^ MC(Decrypt_text[3*4+j]^W[3*4*11+i*4+j],0x09)];
         Encrypt_text[1*4+j] = Inverse_sbox[MC(Decrypt_text[0*4+(j+3)%4]^W[0*4*11+i*4+(j+3)%4],0x09) ^ MC(Decrypt_text[1*4+(j+3)%4]^W[1*4*11+i*4+(j+3)%4],0x0E)^MC(Decrypt_text[2*4+(j+3)%4]^W[2*4*11+i*4+(j+3)%4],0x0B) ^ MC(Decrypt_text[3*4+(j+3)%4]^W[3*4*11+i*4+(j+3)%4],0x0D)];
         Encrypt_text[2*4+j] = Inverse_sbox[MC(Decrypt_text[0*4+(j+2)%4]^W[0*4*11+i*4+(j+2)%4],0x0D) ^ MC(Decrypt_text[1*4+(j+2)%4]^W[1*4*11+i*4+(j+2)%4],0x09)^MC(Decrypt_text[2*4+(j+2)%4]^W[2*4*11+i*4+(j+2)%4],0x0E) ^ MC(Decrypt_text[3*4+(j+2)%4]^W[3*4*11+i*4+(j+2)%4],0x0B)];
         Encrypt_text[3*4+j] = Inverse_sbox[MC(Decrypt_text[0*4+(j+1)%4]^W[0*4*11+i*4+(j+1)%4],0x0B) ^ MC(Decrypt_text[1*4+(j+1)%4]^W[1*4*11+i*4+(j+1)%4],0x0D)^MC(Decrypt_text[2*4+(j+1)%4]^W[2*4*11+i*4+(j+1)%4],0x09) ^ MC(Decrypt_text[3*4+(j+1)%4]^W[3*4*11+i*4+(j+1)%4],0x0E)];

       }

         for(k=0; k<4; k++){
            Decrypt_text[0*4+k] = Encrypt_text[0*4+k];
            Decrypt_text[1*4+k] = Encrypt_text[1*4+k];
            Decrypt_text[2*4+k] = Encrypt_text[2*4+k];
            Decrypt_text[3*4+k] = Encrypt_text[3*4+k];
            }
            print_hex(Decrypt_text);
      }

      i=0;
      for (j=0; j<4; j++){
         Decrypt_text[0*4+j] = Decrypt_text[0*4+j]^W[0*4*11+i*4+j];
         Decrypt_text[1*4+j] = Decrypt_text[1*4+j]^W[1*4*11+i*4+j];   
         Decrypt_text[2*4+j] = Decrypt_text[2*4+j]^W[2*4*11+i*4+j];
         Decrypt_text[3*4+j] = Decrypt_text[3*4+j]^W[3*4*11+i*4+j];
      }
   }

   int main(){  
      unsigned char plainText[16]; 
      unsigned char cipher_key[16]; 
      unsigned char Encrypt_text[16], Decrypt_text[16];
      unsigned long long text[2] = {0x0123456789abcdef,0xfedcba9876543210};
      unsigned long long key[2] = {0x0f1571c947d9e859,0x0cb7add6af7f6798};
      
      
      printf("Encrypt_text\n"); 
      plainText[0*4+0]= (unsigned char)(text[0]>>8*7);   plainText[0*4+1]= (unsigned char)(text[0]>>8*3); 
      plainText[1*4+0]= (unsigned char)(text[0]>>8*6);   plainText[1*4+1]= (unsigned char)(text[0]>>8*2);
      plainText[2*4+0]= (unsigned char)(text[0]>>8*5);   plainText[2*4+1]= (unsigned char)(text[0]>>8*1);
      plainText[3*4+0]= (unsigned char)(text[0]>>8*4);   plainText[3*4+1]= (unsigned char)(text[0]>>8*0);
      
      plainText[0*4+2]= (unsigned char)(text[1]>>8*7);   plainText[0*4+3]= (unsigned char)(text[1]>>8*3); 
      plainText[1*4+2]= (unsigned char)(text[1]>>8*6);   plainText[1*4+3]= (unsigned char)(text[1]>>8*2);
      plainText[2*4+2]= (unsigned char)(text[1]>>8*5);   plainText[2*4+3]= (unsigned char)(text[1]>>8*1);
      plainText[3*4+2]= (unsigned char)(text[1]>>8*4);   plainText[3*4+3]= (unsigned char)(text[1]>>8*0);
      print_hex(plainText);
      
      printf("cipher_key\n"); 
      cipher_key[0*4+0]= (unsigned char)(key[0]>>8*7);   cipher_key[0*4+1]= (unsigned char)(key[0]>>8*3); 
      cipher_key[1*4+0]= (unsigned char)(key[0]>>8*6);   cipher_key[1*4+1]= (unsigned char)(key[0]>>8*2);
      cipher_key[2*4+0]= (unsigned char)(key[0]>>8*5);   cipher_key[2*4+1]= (unsigned char)(key[0]>>8*1);
      cipher_key[3*4+0]= (unsigned char)(key[0]>>8*4);   cipher_key[3*4+1]= (unsigned char)(key[0]>>8*0);
      
      cipher_key[0*4+2]= (unsigned char)(key[1]>>(8*7));  cipher_key[0*4+3]= (unsigned char)(key[1]>>8*3); 
      cipher_key[1*4+2]= (unsigned char)(key[1]>>8*6);    cipher_key[1*4+3]= (unsigned char)(key[1]>>8*2);
      cipher_key[2*4+2]= (unsigned char)(key[1]>>8*5);   cipher_key[2*4+3]= (unsigned char)(key[1]>>8*1);
      cipher_key[3*4+2]= (unsigned char)(key[1]>>8*4);   cipher_key[3*4+3]= (unsigned char)(key[1]>>(8*0));
      print_hex(cipher_key);
      
      printf("AES_Encrypt\n");
      AES_Encrypt(plainText, cipher_key,Encrypt_text);
      print_hex(Encrypt_text);

      printf("AES_Decrypt.\n");
      AES_Decrypt(Encrypt_text, cipher_key,Decrypt_text);
      print_hex(Decrypt_text);

      system("pause");         
   return 0;         
   }