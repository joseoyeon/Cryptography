...
unsigned long long PC2()
{
  ...
  for(i = 0; i < 48; i++, tmp = tmp >> 1LL) if (KEY & (1LL << (56-perm[i]))) res |= tmp; //��ġ 
...
unsigned long long IP(unsigned long long source)
{
  ...
  for(i = 0; i < 64; i++, tmp = tmp >> 1LL) if (source & (1LL << (64-perm[i]))) res |= tmp; //�� ġ 
  return res;
}
...
unsigned rotation(unsigned source, char c, char f)
{
  ...
  else    while(c-- > 0) source = ((source << 1) & 0x0FFFFFFF) | (source >> 27); // ��ȣȭ 
  return source;
}
...
int main()
{  char source_str_char[]= "8CnUCS9U";
unsigned long long source_str = *(unsigned long long*)&source_str_char;
   unsigned long long source_key =0x000000000002ae78;
